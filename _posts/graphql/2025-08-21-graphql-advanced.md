---
layout: post
title: 'GraphQL 공부 2: 심화 개념과 실전 예제'
...
{
  me: user(id: 1) { ...UserFields }
  you: user(id: 2) { ...UserFields }
}
```
...
const root = {
  users: (): User[] => users,
  user: ({ id }: { id: number }): User | undefined => users.find(u => u.id === id),
  addUser: ({ input }: { input: { name: string; role?: 'USER' | 'ADMIN' } }): User => {
    const newUser: User = { id: users.length + 1, name: input.name, role: input.role ?? 'USER' };
    users.push(newUser);
    return newUser;
  }
};
...
- <a href="https://graphql.org/learn/" target="_blank" rel="noopener">GraphQL Learn</a>
