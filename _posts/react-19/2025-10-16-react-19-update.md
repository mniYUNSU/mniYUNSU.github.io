---
layout: post
title: React 19 핵심 업데이트 정리
date: 2025-10-16 00:00:00 +0900
categories: [React Update]
tags: [React, Release Notes, Frontend]
description: React 19의 주요 기능을 개요부터 기대 효과까지 체계적으로 정리한 글이다.
---

## 개요

React 19는 2024년 말 공개된 RC를 바탕으로 정식 릴리스를 준비하며 폼 제출, 서버 렌더링, 비동기 데이터 처리, 문서 메타데이터, Web Components 호환성, 오류 진단 등 실무에서 자주 사용하는 기능을 다듬었다. 이번 글에서는 핵심 업데이트를 초보 개발자도 이해하기 좋은 흐름으로 정리한다.

## 어떤 기능이 업데이트 되었는지에 대한 간략한 설명

- Actions와 폼 관련 훅: `form` 태그에 비동기 액션을 연결하고 제출 상태를 추적한다.
- React Server Components 안정화: 서버 전용 컴포넌트를 공식 패키지로 묶어 스트리밍 렌더링을 지원한다.
- 새 `use` 훅: Promise나 Context를 동기적으로 읽어 Suspense와 연계한다.
- Document Metadata API: 컴포넌트에서 `<title>`, `<meta>`, `<link>`를 선언적으로 관리한다.
- Web Components 호환성 개선: 커스텀 엘리먼트와 React props·이벤트를 자연스럽게 연결한다.
...
#### 업데이트 기능은 기존과 비교해서 어떤게 다른가?

React 18까지는 `onSubmit` 핸들러와 `useState` 조합으로 로딩·에러·성공 상태를 직접 관리해야 했다. React 19에서는 `form`의 `action` 속성에 비동기 함수를 연결하면 `useActionState`, `useFormStatus`, `useOptimistic`가 제출 상태와 낙관적 UI를 자동으로 돕는다. 서버 전용 액션도 동일한 문법을 사용한다.

#### 업데이트 기능의 간단 사용 예제

```jsx
// app/FeedbackForm.jsx
"use client";
...
async function submitFeedback(prevState, formData) {
  ...
  if (!message) {
    return { success: false, error: "피드백을 입력해 달라." };
  }
...
export function FeedbackForm() {
  ...
  return (
    <form action={submitAction} className="feedback-form">
      <textarea name="message" placeholder="의견을 남겨 달라" />
      <SubmitButton />
      {state.error && <p className="error">{state.error}</p>}
      {state.success && <p className="success">소중한 의견 고맙다!</p>}
    </form>
  );
}
...
- 제출 로직과 상태 변화를 한 액션 함수에 모아 코드가 단순해진다.
- 낙관적 UI와 에러 표시를 기본 훅으로 처리해 사용자 경험이 안정적이다.
...
export default function WeatherPage() {
  return (
    <Suspense fallback={<p>날씨 정보를 불러오는 중이다…</p>}>
      <WeatherPanel />
    </Suspense>
  );
}
...
- `useEffect`와 `setState` 보일러플레이트가 사라져 코드가 간결하다.
...
export default async function ArticlePage({ params }) {
  ...
  return (
    <>
      <title>{article.title} | 프런트 매거진</title>
      <meta name="description" content={article.summary} />
      <link rel="canonical" href={`https://example.com/articles/${params.slug}`} />
...
React 18 이전에는 커스텀 엘리먼트에 함수형 props를 전달하거나 이벤트를 연결할 때 제약이 있었다. React 19는 커스텀 엘리먼트와 React props·이벤트·`ref`를 자연스럽게 연결한다.
...
export default function RatingSection() {
  ...
  return (
    <section>
      <h2>서비스 만족도를 평가해 달라</h2>
      <rating-widget value={4} onrate={handleRate} ref={(node) => node?.focus()} />
    </section>
  );
}
```
...
원래는 조건부 렌더링으로 UI를 숨기면 컴포넌트가 언마운트되어 내부 상태와 Effect가 사라졌다. React 19의 `<Activity>`는 `mode` 값만 전환해도 DOM을 `display: "none"`으로 숨기면서 Effects를 정리하고, 다시 보일 때는 직전에 저장한 state를 복원한다. 게다가 처음부터 `hidden`으로 렌더링하면 화면에는 보이지 않지만 코드는 저우선순위로 실행돼 사전 로딩이 가능하다.
...
- 숨겨진 컴포넌트의 state를 유지하므로 다시 표시할 때 초기화 과정을 생략한다.
...
React 19는 폼 제출, 서버 렌더링, 비동기 데이터 흐름, 메타데이터, Web Components, 오류 진단 같은 실무 요소를 핵심 업데이트로 정리했다. Actions로 제출 경험을 정돈하고, 서버 컴포넌트와 `use` 훅으로 데이터 경계를 명확히 나누며, 메타데이터와 Web Components 개선으로 기존 자산까지 유연하게 활용할 수 있다.

## 참조문서

- [React Blog – React 19](https://react.dev/blog/2024/12/05/react-19)
- [React Docs – Actions & Forms](https://react.dev/reference/react-dom/hooks/useFormStatus)
- [React Docs – React Server Components](https://react.dev/blog/2023/03/22/react-labs-what-we-have-been-working-on-march-2023#react-server-components)
- [React Docs – `use` 훅](https://react.dev/reference/react/use)
- [React Docs – Document Metadata](https://react.dev/reference/react-dom/components#document-metadata)
...
- [React Docs – Web Components](https://react.dev/reference/react-dom/components/common#web-components)
