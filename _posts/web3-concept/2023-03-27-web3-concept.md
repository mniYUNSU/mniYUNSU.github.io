...
- **트위터의 특정 계정 영구정지**<br>
  트위터는 2021년 **도널드 트럼프 미국 대통령**의 트위터 계정을 영구정지 조치했다. 근거는 "추가로 폭력을 조장할 위험" 이었다. 이와 관련해 잘했다는 의견이 그 당시 지배적이었다.<br><br>그러나, 독일의 메르켈 총리는 "**표현의 자유**라는 기본권은 근본적인 권리로 법률과 입법기관이 정의내린 테두리 내에서 제한될 수 있으며, **소셜미디어 플랫폼 기업의 결정에 따라서는 안 된다.**" 라며 트럼프가 아닌 **테크 기업을 향해** 메세지를 던졌다.
...
사용자들의 온라인 활동에서 발생하는 막대한 트래픽을 일부의 기업들은 이를 광고에 접목해 **광고 기반 수익 모델**을 탄생시키기도 했다.
...
개빈 우드는 이러한 부분에 문제 의식을 갖고 Web3.0을 통해 이를 개선하고자 했다.
...
Web3를 사용하기 위한 기술 진입 장벽은 높다. 보안에 대한 문제를 인식해야 하고, 복잡한 기술 문서를 이해해야 한다. 또한 더 직관적인 UI/UX를 제공해야 한다. 또한, 기존에는 데이터를 중앙 서버에서 REST API 호출을 통해 가져왔다면, Web3는 블록체인 **노드와의 상호 작용**으로 데이터에 접근하므로 Web2.0 서비스보다 느린 속도를 경험할 수 있다.
